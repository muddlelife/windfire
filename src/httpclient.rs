use crate::FINGER_DATA;
use crate::utils::{get_favicon_url, get_fofa_iconhash, get_format_info};
...
pub async fn send_request(
    ...
    if u16_vec.contains(&0) || u16_vec.contains(&status_code) {
        // 说明全部打印 ,然后进行指纹匹配，如果能匹配到，则返回cms，遍历指纹
        let mut cms_list: Vec<String> = Vec::new();

        // shiro判断
        if header.contains("set-cookie: rememberMe=deleteMe;") {
            cms_list.push("Shiro".to_string());
        }
